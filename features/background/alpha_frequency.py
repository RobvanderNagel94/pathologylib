...
def find_two_highest_peaks(Sxx_log: ndarray) -> (float, float, float, float):
    """
    Find two highest peaks in the log transformed power spectrum `Sxx_log`.
    Returns peaks in terms of the peak amplitudes `A1` and `A2` and
    ...
    """
    # Find initial peaks and their relative importance
    ...
    # Extract first and second most significant peak’s locations and amplitudes
...
def fit_spectral_curve(Sxx_log: ndarray,
                       f: ndarray,
                       f1: float,
                       f2: float,
                       A1: float,
    ...
    """
    Performs a least-squares fit to localize the two most dominant peak locations,
...
def compute_correlation_ratio(Sxx_log: ndarray,
                              B: float,
                              C: float,
                              f1: float,
                              f2: float,
    ...
    # Find frequency indices for peaks f1 -+ d1 and f2 -+ d2.
    idx = ((f1 - d1 <= x.index) & (x.index <= f1 + d1)) | ((f2 - d2 <= x.index) & (x.index <= f2 + d2))

    # Delete peaks from original log spectrum.
    Pexcl[idx] = 0
    Pres = np.copy(Pexcl)

    # Fill empty spaces with background estimate.
...
def estimate_alpha_rhythm(A1: ndarray,
                          A2: ndarray,
                          f1: ndarray,
                          f2: ndarray,
                          d1: ndarray,
